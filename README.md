...
``` bash
curl -s https://raw.githubusercontent.com/AhmedMohamedEid/odoo-18-docker-compose/master/run.sh | sudo bash -s odoo-one 10018 20018
```
and/or run the following to set up another Odoo instance @ `localhost:11018` (default master password: `minhng.info`):

``` bash
curl -s https://raw.githubusercontent.com/AhmedMohamedEid/odoo-18-docker-compose/master/run.sh | sudo bash -s odoo-two 11018 21018
```
